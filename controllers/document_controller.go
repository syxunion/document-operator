...
func (r *DocumentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		// 如果没有实例就要创建了
		if errors.IsNotFound(err) {
			log.Info("instance Not Found, create instance")

			// 立即创建deployment
			if err = createDocumentDeployment(ctx, instance, r); err != nil {
				log.Error(err, "Deployment Not Found,create Deployment")
				// 返回错误信息给外部
				return ctrl.Result{}, err
			}
			...
			if err = createService(ctx, instance, r, req); err != nil {
				log.Error(err, "Service Not Found,create Service")
				// 返回错误信息给外部
				return ctrl.Result{}, err
			}
			...
			if err = createRoute(ctx, instance, r, req); err != nil {
				log.Error(err, "Route Not Found,create Route")
				// 返回错误信息给外部
				return ctrl.Result{}, err
			}
